...
static void *DMAbuffer_va_tfa98xx;
...
unsigned short tfa98xx_vol = 0;
bool tfa_vol_force = false;

/*oppo 2016-07-20 add by zhangping for tfa9890 force calibrate*/
...
int NXPspk_i2c_tfa98xx_write(struct i2c_client *client, const u8 *txbuf, u16 len)
{
	int i = 0;
	int ret;
	char *TfaI2CDMABuf = (char *)DMAbuffer_va_tfa98xx;

	for (i = 0;  i < len; i++)
		TfaI2CDMABuf[i] = txbuf[i];

	if (len <= 8)
		ret = i2c_master_send(tfa98xx_whole->i2c, txbuf, len);
	else
		ret = i2c_master_send(tfa98xx_whole->i2c, (char *)TfaI2CDMABuf, len);
...
static ssize_t AudDrv_nxpspk_write(struct file *fp, const char __user *data, size_t count, loff_t *offset)
{
	...
	if (count <= 8)
		ret = i2c_master_send(tfa98xx_whole->i2c, tmp, count);
	else
		ret = i2c_master_send(tfa98xx_whole->i2c, (char *)TfaI2CDMABuf, count);
...
static ssize_t AudDrv_nxpspk_read(struct file *fp,  char __user *data, size_t count, loff_t *offset)
{
	...
	if (count <= 8)
		ret = i2c_master_recv(tfa98xx_whole->i2c, tmp, count);
	else {
		ret = i2c_master_recv(tfa98xx_whole->i2c, (char *)TfaI2CDMABuf, count);
		for (i = 0; i < count; i++)
...
static int tfa98xx_volume_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("%s: dsp status = %d\n", __func__, tfa98xx->dsp_init);
	while((tfa98xx->dsp_init == TFA98XX_DSP_INIT_NOT_DONE) && (--waitcnt)) {
		msleep(5);
	}
	if(tfa98xx->dsp_init != TFA98XX_DSP_INIT_NOT_DONE) {
		mutex_lock(&tfa98xx->dsp_lock);
		tfa98xx_open(tfa98xx->handle);
	    err = tfa98xx_get_volume_level(tfa98xx->handle, &tfa98xx_vol);
		printk("%s: volume :%d\n", __func__, tfa98xx_vol);
		ucontrol->value.integer.value[0] = tfa98xx_vol;
		tfa98xx_close(tfa98xx->handle);
		mutex_unlock(&tfa98xx->dsp_lock);
		return 0;
	} else {
		printk("%s: wait for pending more than 500ms error\n", __func__);
		return -1;
	}
}
 ...
 */
	...
	if (tfa98xx->dsp_init != TFA98XX_DSP_INIT_NOT_DONE) {
		mutex_lock(&tfa98xx->dsp_lock);
		tfa98xx_open(tfa98xx->handle);
		err = tfa98xx_set_volume_level(tfa98xx->handle, tfa98xx_vol);
		if (err)
			printk("%s: An error occured when set volume level, code:%d", __func__, err);
		tfa98xx_close(tfa98xx->handle);
		mutex_unlock(&tfa98xx->dsp_lock);
		printk("%s: set vol end\n", __func__);
		return 0;
	} else {
		printk("%s: wait for pending more than 500ms error\n", __func__);
		return -1;
...
/*oppo 2016-07-20 add by zhangping for tfa9890 force calibrate*/
	...
	#endif
/*oppo 2016-07-20 add by zhangping for tfa9890 force calibrate end*/
	mutex_lock(&tfa98xx->dsp_lock);
...
//	pr_debug("selected container profile [%d]\n", prof_idx);
    printk("%s: selected container profile [%d]\n", __func__, prof_idx);
...
static int tfa98xx_register_dsp(struct tfa98xx *tfa98xx)
{
    ...
    printk("%s: tfa98xx_cnt_max_device is: %d\n", __func__, tfa98xx_cnt_max_device());
...
retry:
#ifdef VENDOR_EDIT
		ret = NXPspk_i2c_tfa98xx_write(tfa98xx->i2c, data, len);
#else
 ...
 * modify for TFA firmware multi-project shared baseline */
	...
	printk("%s: tfa_load_cnt result: %d\n", __func__, ret);
...
/* yongzhi.zhang@MultiMedia.AudioDriver.SmartPA, 2017/01/17, Add for tfa9890 and tfa9891 compatibility */
	...
	int retries = I2C_RETRIES;
retry:
	ret = regmap_read(tfa98xx->regmap, TFA98XX_REVISIONNUMBER, &reg);
	reg = reg & 0xff;
	if ((ret < 0) || !is_tfa98xx_series(reg)) {
...
static void tfa98xx_monitor(struct work_struct *work)
{
	...
	if (TFA_GET_BF(tfa98xx->handle, PWDN) == 0) {
		...
		for (regnum = 0; regnum <= TFA98XX_I2S_SEL_REG; regnum++) {
			...
			pr_info("TFA reg 0x%x, value 0x%x\n", regnum, val_reglist);
		}
	}
	...
	if (ret) {
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_err("tfa98xx_open failed %d in monitor\n", ret);
		return;
	}
	ret = tfa98xx_get_mtp(tfa98xx->handle, &status);
	if (ret) {
		ret = -EIO;
		pr_err("tfa98xx_get_mtp failed %d in monitor\n", ret);
		goto rc_moni_err;
	}
	...
	if (ret) {
		ret = -EIO;
		pr_err("tfaRunWaitCalibration failed %d in monitor\n", ret);
		goto rc_moni_err;
	}
	str = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!str) {
	...
	case -1:
		...
		break;
	default:
		pr_err("Unknown calibration status in monitor: %d\n", calibrate_done);
		ret = -EINVAL;
	}
	pr_info("calib_done in monitor: %d - ret = %d - %s", calibrate_done, ret, str);
	kfree(str);
...
static int tfa98xx_late_fw_load(void)
{
    ...
    printk("%s:\n", __func__);
	ret = tfa98xx_load_container(tfa98xx_whole);
...
retry:
#ifdef VENDOR_EDIT
	ret = NXPspk_i2c_tfa98xx_write(tfa98xx->i2c, data, count+1);
#else
...
/* yongzhi.zhang@MultiMedia.AudioDriver.SmartPA, 2016/12/07, modify for synchronizing audioout and tfa_start */
	...
	tfarstdefault = pinctrl_lookup_state(pinctrltfa, "tfa98xx_reset_default");
	if (IS_ERR(tfarstdefault)) {
		ret = PTR_ERR(tfarstdefault);
		pr_err("%s pinctrl_lookup_state tfa98xx_reset_default fail %d\n", __func__, ret);
	}
...
{
	pr_info("AudDrv_nxpspk_probe\n");

	DMAbuffer_va_tfa98xx = dma_alloc_coherent(&platform->dev, 4096, &DMAbuffer_pa_tfa98xx, GFP_KERNEL);
	if (DMAbuffer_va_tfa98xx == NULL) {
		pr_err("AudDrv_nxpspk_probe 4096 dma_alloc_coherent error !!!!!\n");
		return -1;
	}

	pr_info("AudDrv_nxpspk_probe dma_alloc_coherent TfaI2CDMABuf_va = %p\n", DMAbuffer_va_tfa98xx);
...
{
	pr_info("+AudDrv_nxpspk_remove\n");

	if (DMAbuffer_va_tfa98xx != NULL) {
		dma_free_coherent(&platform->dev, 4096, DMAbuffer_va_tfa98xx, DMAbuffer_pa_tfa98xx);
		DMAbuffer_va_tfa98xx = NULL;
		DMAbuffer_pa_tfa98xx = 0;
	}
...
static int __init tfa98xx_i2c_init(void)
{
	...
	if ((get_project() == 18011) || (get_project() == 18311)) {
		return 0;
	}
#endif /* VENDOR_EDIT */
	...
	if (ret) {
		pr_warn("i2c_add_driver Fail:%d\n", ret);
		return ret;
	} else {
		pr_warn("i2c_add_driver success:%d\n", ret);
	}
...
#ifndef VENDOR_EDIT
subsys_initcall(tfa98xx_i2c_init);
#else
