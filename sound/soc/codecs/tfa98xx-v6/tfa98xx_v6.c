...
retry:
	...
	if (ret < 0 || !is_tfa98xx_series_v6(rev)) {
...
{// /proc/tfa98xx/ftm_tfa98xx_v6
	const int size = 1024;
	char buffer[size];
	int n = 0;
	int hw_flag = 0;

	hw_flag = main_hwid6_val;
	n += scnprintf(buffer + n, size - n, "%d ", hw_flag);
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		...
		if (fwload_cnt_v6 < FW_LOADING_RETRY_TIMES) {
			...
			if(fwload_comm_cnt_v6 < FW_LOADING_RETRY_TIMES) {
				pr_err("%s: read comm %s, try: %d\n", __func__, fw_comm_name, fwload_comm_cnt_v6);
				request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
...
retry:
	...
	if ((get_project() == 17197) || (get_project() == 18311) || (get_project() == 18011)) {
		if(reg == 0x94) {/* tfa9894 */
...
static int __init tfa98xx_i2c_init(void)
{
	...
	if (get_project() == 17197) {
		...
		if ((get_project() != 18311) && (get_project() != 18011)) {
			return 0;
		}
	}
#endif /* VENDOR_EDIT */
