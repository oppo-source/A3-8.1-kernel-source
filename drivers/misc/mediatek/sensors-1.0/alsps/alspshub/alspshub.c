...
static int ps_cali_result = 0;
static ssize_t ps_show_cali(struct device_driver *ddri, char *buf)
{
	int temp_offset[2] = {0};
	get_sensor_parameter(ID_PROXIMITY ,temp_offset);
	APS_PR_ERR("ps_show_cali %d\n",ps_cali_result);
	return scnprintf(buf, PAGE_SIZE, "%d,%d\n", ps_cali_result, temp_offset[0]);
}
...
static ssize_t ps_store_cali(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d", &calib)) {
		APS_PR_ERR("ps_store_cali %d\n",calib);
		if (calib==1) {
			sensor_set_cmd_to_hub(ID_PROXIMITY, CUST_ACTION_SET_CALI, (void*)&offset); //clear offset
			...
			if(!res && (offset >= 0)){
				...
				temp_offset[0] = offset;
				sensor_set_cmd_to_hub(ID_PROXIMITY, CUST_ACTION_SET_CALI, (void*)&offset);
				update_sensor_parameter(ID_PROXIMITY ,temp_offset);
			}else{
				ps_cali_result = -1;
			}

			APS_PR_ERR("%s:res=%d, offset=%d\n",__func__,res,offset);
		}
	} else {
 		APS_PR_ERR("invalid content: '%s', length = %zd\n", buf, count);
 	}
 ...
 static ssize_t ps_show_offset(struct device_driver *ddri, char *buf)
 {
	int offset[2] = {0};
 ...
 static ssize_t ps_store_offset(struct device_driver *ddri, const char *buf, size_t count)
 {
	int offset[2] = {0};

	if (1 == sscanf(buf, "%d %d", &offset[0], &offset[1])) {
		APS_PR_ERR("ps_store_offset offset=%d  %d\n", offset[0], offset[1]);
 	} else {
		APS_PR_ERR("ps_store_offset invalid content: '%s', length = %zd\n", buf, count);
 	}

	update_sensor_parameter(ID_PROXIMITY ,offset);
	sensor_set_cmd_to_hub(ID_PROXIMITY, CUST_ACTION_SET_CALI, (void*)offset);
...
static DRIVER_ATTR(offset, S_IWUSR | S_IRUGO, ps_show_offset, ps_store_offset );
...
static int als_recv_data(struct data_unit_t *event, void *reserved)
{
    ...
    recv_num++;
    if(recv_num >= 60){
        APS_PR_ERR("get %d Report lux : %d\n", recv_num,event->light);
        recv_num = 0;
    }
    #endif
