...
static unsigned int init_sensor_calibraiton_paramater(void)
{
	...
	DEVINF_ERR("gain_als(%d) prox_offset_L(%d) prox_offset_H(%d) gsensor_offset(%d %d %d) gyro_offset(%d %d %d)\n",
		SensorData->gain_als,SensorData->prox_offset[0],SensorData->prox_offset[0],
...
int update_sensor_parameter(int sensortye ,int * data)
{
	...
	DEVINF_ERR("gain_als(%d) prox_offset_L(%d) prox_offset_H(%d) gsensor_offset(%d %d %d) gyro_offset(%d %d %d)\n",
		SensorData->gain_als,SensorData->prox_offset[0],SensorData->prox_offset[1],
		SensorData->gsensor_offset[0],SensorData->gsensor_offset[1],SensorData->gsensor_offset[2],
		SensorData->gyro_offset[0],SensorData->gyro_offset[1],SensorData->gyro_offset[2]);
...
/* Fuchun.Liao@BSP.CHG.Basic 2018/08/08 modify for sensor i2c error workaround */
		...
		if (temp_cali[0] >= 0)
		{
			err = sensor_set_cmd_to_hub(ID_PROXIMITY, CUST_ACTION_SET_CALI, (void*)temp_cali);
			DEVINF_ERR("set ps factory cali=%d, delta=%d, res=%d\n",temp_cali[0], temp_cali[1], err);
		}
	}
RETRY_GET_SENSOR:
	if ((retry < 3) && err) {
