...
static inline void cldma_write32_ao_misc(struct md_cd_ctrl *md_ctrl, u32 reg, u32 val)
{
#if (MD_GENERATION == 6293)
	u32 reg2, reg2_val;
...
#endif
	CCCI_DEBUG_LOG(MD_SYS1, TAG, "%s:%pa, 0x%x, val=0x%x\n", __func__, &data_ptr, tbd->msb.msb_byte, val);
}
...
void md_cd_traffic_monitor_func(unsigned long data)
{
	...
	CCCI_REPEAT_LOG(md_ctrl->md_id, TAG, "Tx active %d\n", md_ctrl->txq_active);
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++) {
		if (md_ctrl->txq[i].busy_count != 0) {
			CCCI_REPEAT_LOG(md_ctrl->md_id, TAG, "Txq%d busy count %d\n", i, md_ctrl->txq[i].busy_count);
			md_ctrl->txq[i].busy_count = 0;
		}
		CCCI_REPEAT_LOG(md_ctrl->md_id, TAG, "Tx:%d-%d\n",
	...
	for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
		q_rx_rem_nsec[i] = (tinfo->latest_q_rx_isr_time[i] == 0 ?
			0 : do_div(tinfo->latest_q_rx_isr_time[i], 1000000000));
		CCCI_REPEAT_LOG(md_ctrl->md_id, TAG, "RX:%lu.%06lu, %d\n",
			(unsigned long)tinfo->latest_q_rx_isr_time[i], q_rx_rem_nsec[i] / 1000,
			md_ctrl->rx_traffic_monitor[i]);
	}

#ifdef ENABLE_CLDMA_TIMER
	CCCI_REPEAT_LOG(md_ctrl->md_id, TAG, "traffic(tx_timer): [3]%llu %llu, [4]%llu %llu, [5]%llu %llu\n",
...
static void cldma_dump_packet_history(struct md_cd_ctrl *md_ctrl)
{
	...
	ccci_md_dump_log_history(md_ctrl->md_id, &md_ctrl->traffic_info, 1, QUEUE_LEN(md_ctrl->txq),
		QUEUE_LEN(md_ctrl->rxq));
}

static void cldma_dump_queue_history(struct md_cd_ctrl *md_ctrl, unsigned int qno)
{
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Current txq%d pos: tr_done=%x, tx_xmit=%x\n", qno,
...
#error 6293 should not enable fast header!
		...
		if (ret >= 0 || ret == -CCCI_ERR_DROP_PACKET) {
			...
			ccci_md_check_rx_seq_num(md_ctrl->md_id, &md_ctrl->traffic_info, &ccci_h, queue->index);
#endif
			/* refill */
			req = queue->rx_refill;
			rgpd = (struct cldma_rgpd *)req->gpd;
...
static int cldma_net_rx_push_thread(void *arg)
{
	...
	while (1) {
		if (skb_queue_empty(&queue->skb_list.skb_list)) {
			cldma_queue_broadcast_state(md_ctrl, RX_FLUSH, IN, queue->index);
			count = 0;
		...
		count++;
#ifdef CCCI_SKB_TRACE
		per_md_data->netif_rx_profile[6] = sched_clock() - per_md_data->netif_rx_profile[6];
...
static void cldma_rx_done(struct work_struct *work)
{
	...
	cldma_write32_ao_misc(md_ctrl, CLDMA_AP_L2RIMCR0,
			...
			(CLDMA_RX_INT_QUEUE_EMPTY & ((1 << queue->index) << CLDMA_RX_QE_OFFSET)));
}
...
//Yongyao.Song@PSW.NW.PWR.1053636, 2017/08/01, add for modem wake up source
	...
	while (1) {
		...
		list_for_each_entry(req_bd, &req->bd, entry) {
			...
			if (tbd->non_used == 1) {
				tbd->non_used = 2;
				dma_unmap_single(ccci_md_get_dev_by_id(md_ctrl->md_id), req_bd->data_buffer_ptr_saved,
		...
		count++;
#if MD_GENERATION >= (6293)
		ccci_h = (struct ccci_header *)(skb_push(skb_free, sizeof(struct ccci_header)));
#else
...
//Yongyao.Song@PSW.NW.PWR.1053636, 2017/08/01, add for modem wake up source
	...
	while (1) {
		 ...
		 */
		dma_unmap_single(ccci_md_get_dev_by_id(md_ctrl->md_id), dma_free, dma_len, DMA_TO_DEVICE);
#if MD_GENERATION >= (6293)
		ccci_h = (struct ccci_header *)(skb_push(skb_free, sizeof(struct ccci_header)));
#else
		ccci_h = (struct ccci_header *)skb_free->data;
...
static void cldma_tx_done(struct work_struct *work)
{
	...
	if (L2TISAR0 & CLDMA_TX_INT_QUEUE_EMPTY & ((1 << queue->index) << CLDMA_TX_QE_OFFSET))
		cldma_tx_queue_empty_handler(queue);
	if (L2TISAR0 & CLDMA_TX_INT_DONE & (1 << queue->index)) {
		/* ACK interrupt */
...
static void cldma_rx_ring_init(struct md_cd_ctrl *md_ctrl, struct cldma_ring *ring)
{
	int i;
...
static void cldma_tx_ring_init(struct md_cd_ctrl *md_ctrl, struct cldma_ring *ring)
{
	int i, j;
	...
	if (ring->type == RING_GPD_BD) {
		...
		cldma_tgpd_set_next_ptr(tgpd, first_item->gpd_addr);
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "ring=%p -> gpd_ring=%p\n", ring, &ring->gpd_ring);
	}
}
...
static void cldma_queue_switch_ring(struct md_cd_queue *queue)
{
	...
	if (queue->dir == OUT) {
		if ((1 << queue->index) & NET_TX_QUEUE_MASK) {
			if (!ccci_md_in_ee_dump(md_ctrl->md_id))	/* normal mode */
		...
		if ((1 << queue->index) & NET_RX_QUEUE_MASK) {
			...
			else if ((1 << queue->index) & NORMAL_RX_QUEUE_MASK)	/* if this queue has exception mode */
	...
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "queue %d/%d switch ring to %p\n",
...
static void cldma_irq_work_cb(struct md_cd_ctrl *md_ctrl)
{
	...
	if (atomic_read(&md_ctrl->wakeup_src) == 1)
		CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "wake up by CLDMA_MD L2(%x/%x)(%x/%x)!\n",
						L2TISAR0, L2RISAR0, L2TIMR0, L2RIMR0);
	else
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "CLDMA IRQ L2(%x/%x)(%x/%x)!\n",
						L2TISAR0, L2RISAR0, L2TIMR0, L2RIMR0);
...
#if MD_GENERATION <= (6292)
	if (unlikely(!(L2RISAR0 & CLDMA_RX_INT_DONE) && !(L2TISAR0 & CLDMA_TX_INT_DONE) &&
			!(L2RISAR0 & CLDMA_RX_INT_QUEUE_EMPTY) && !(L2TISAR0 & CLDMA_TX_INT_QUEUE_EMPTY))) {
	...
	if (L2RISAR0) {
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			if ((L2RISAR0 & CLDMA_RX_INT_DONE & (1 << i)) ||
			    (L2RISAR0 & CLDMA_RX_INT_QUEUE_EMPTY & ((1 << i) << CLDMA_RX_QE_OFFSET))) {
				...
				cldma_write32_ao_misc(md_ctrl, CLDMA_AP_L2RIMSR0,
					      ...
					      (CLDMA_RX_INT_QUEUE_EMPTY & ((1 << i) << CLDMA_RX_QE_OFFSET)));
...
void cldma_stop(unsigned char hif_id)
{
	...
	struct ccci_smem_region *mdccci_dbg = ccci_md_get_smem_by_user_id(md_ctrl->md_id, SMEM_USER_RAW_MDCCCI_DBG);
	struct ccci_smem_region *mdss_dbg = ccci_md_get_smem_by_user_id(md_ctrl->md_id, SMEM_USER_RAW_MDSS_DBG);
	struct ccci_per_md *per_md_data = ccci_get_per_md_data(md_ctrl->md_id);
	...
	do {
		...
		if ((++count) % 100000 == 0) {
			...
			CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Dump MD EX log\n");
	...
	cldma_write32_ao_misc(md_ctrl, CLDMA_AP_L2RIMSR0, CLDMA_BM_INT_ALL);
#if MD_GENERATION <= (6292)
...
void cldma_stop_for_ee(unsigned char hif_id)
{
	...
	cldma_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L2TIMSR0, CLDMA_BM_INT_ALL);
	cldma_write32_ao_misc(md_ctrl, CLDMA_AP_L2RIMSR0,
		      (CLDMA_RX_INT_DONE | CLDMA_RX_INT_QUEUE_EMPTY | CLDMA_RX_INT_ERROR) & NONSTOP_QUEUE_MASK_32);
#if MD_GENERATION <= (6292)
	cldma_write32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_L2TIMSR1, CLDMA_BM_INT_ALL);
	cldma_write32(md_ctrl->cldma_ap_ao_base, CLDMA_AP_L2RIMSR1, CLDMA_BM_INT_ALL);
#endif
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			...
			list_for_each_entry(req, &md_ctrl->rxq[i].tr_ring->gpd_ring, entry) {
				...
				if (req->skb == NULL) {
					...
					/*which queue*/
					CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "skb NULL in Rx queue %d/%d\n",
							i, queue->index);
...
static int md_cd_stop_queue(unsigned char hif_id, unsigned char qno, DIRECTION dir)
{
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)ccci_hif_get_by_id(hif_id);
	int count, ret;
	unsigned long flags;
	...
	if (dir == IN) {
		...
		cldma_write32_ao_misc(md_ctrl, CLDMA_AP_L2RIMSR0,
				...
				(CLDMA_RX_INT_QUEUE_EMPTY & ((1 << qno) << CLDMA_RX_QE_OFFSET)));
		count = 0;
		...
		do {
			...
			CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "stop Rx CLDMA queue %d, status=%x, count=%d\n", qno, ret,
				     count++);
		} while (ret != 0);
		spin_unlock_irqrestore(&md_ctrl->cldma_timeout_lock, flags);
	}
...
static int md_cd_start_queue(unsigned char hif_id, unsigned char qno, DIRECTION dir)
{
	struct md_cd_ctrl *md_ctrl = (struct md_cd_ctrl *)ccci_hif_get_by_id(hif_id);
	struct cldma_request *req = NULL;
	struct cldma_rgpd *rgpd;
	...
	if (dir == IN) {
		...
		if (md_state != WAITING_TO_STOP && md_state != GATED && md_state != INVALID) {
...
static inline int cldma_sw_init(struct md_cd_ctrl *md_ctrl)
{
	...
	/* request IRQ */
	ret = request_irq(md_ctrl->cldma_irq_id, cldma_isr, md_ctrl->cldma_irq_flags, "CLDMA_AP", md_ctrl);
	if (ret) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "request CLDMA_AP IRQ(%d) error %d\n", md_ctrl->cldma_irq_id,
			     ret);
		return ret;
	}
	cldma_disable_irq(md_ctrl);
...
static int md_cd_give_more(unsigned char hif_id, unsigned char qno)
{
	...
	if (qno >= QUEUE_LEN(md_ctrl->rxq))
		return -CCCI_ERR_INVALID_QUEUE_INDEX;
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "give more on queue %d work %p\n", qno, &md_ctrl->rxq[qno].cldma_rx_work);
	spin_lock_irqsave(&md_ctrl->cldma_timeout_lock, flags);
...
void md_cldma_clear(unsigned char hif_id)
{
	...
	for (i = 0; (CLDMA_BM_ALL_QUEUE & ret) && i < QUEUE_LEN(md_ctrl->rxq); i++) {
		if ((CLDMA_BM_ALL_QUEUE & ret) & (1 << i)) {
			CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "MD CLDMA txq=%d is active, need AP rx collect!", i);
	...
	if (retry == 0 && cldma_read32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_CLDMA_IP_BUSY) != 0) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "md_cldma_clear: wait md tx done failed.\n");
		md_cd_traffic_monitor_func((unsigned long)md_ctrl);
		cldma_dump_register(md_ctrl);
	} else {
		CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "md_cldma_clear: md tx done\n");
	}
	cldma_stop(hif_id);
					  ...
					  struct sk_buff *skb, unsigned int ioc_override)
{
...
#endif
	/* network does not has IOC override needs */
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "SGIO, GPD=%p, frags=%d, len=%d, headlen=%d\n", tx_req->gpd,
	...
	for (cur_frag = -1; cur_frag < info->nr_frags; cur_frag++) {
		...
		tbd = tx_req_bd->gpd;
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "SGIO, BD=%p, frag%d, frag_len=%d\n", tbd, cur_frag, frag_len);
		/* update BD */
...
static int md_cd_send_skb(unsigned char hif_id, int qno, struct sk_buff *skb,
	int skb_from_pool, int blocking)
{
	...
	if (skb_from_pool && skb_headroom(skb) == NET_SKB_PAD) {
		...
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "send request: skb %p use default value!\n", skb);
 ...
 retry:
	...
	if (queue->budget > 0 && tx_req->skb == NULL) {
		...
		/* check CLDMA status */
		if (cldma_read32(md_ctrl->cldma_ap_pdn_base, CLDMA_AP_UL_STATUS) & (1 << qno)) {
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "ch=%d qno=%d free slot 0, CLDMA_AP_UL_STATUS=0x%x\n",
...
int ccci_cldma_hif_init(unsigned char hif_id, unsigned char md_id)
{
	...
	struct md_hw_info *md_info = (struct md_hw_info *)ccci_md_get_hw_info(md_id);
	struct cldma_hw_info *hw_info = (struct cldma_hw_info *)md_info->hif_hw_info;
	int i;

	md_ctrl = kzalloc(sizeof(struct md_cd_ctrl), GFP_KERNEL);
	if (md_ctrl == NULL) {
		CCCI_ERROR_LOG(-1, TAG, "ccci_cldma_hif_init:alloc md_ctrl fail\n");
		return -1;
	}
	...
	md_ctrl->cldma_ap_pdn_base = (void __iomem *)(hw_info->cldma_ap_pdn_base);
	md_ctrl->cldma_ap_ao_base = (void __iomem *)(hw_info->cldma_ap_ao_base);
#if MD_GENERATION <= (6292)
	md_ctrl->cldma_md_pdn_base = (void __iomem *)(hw_info->cldma_md_pdn_base);
	md_ctrl->cldma_md_ao_base = (void __iomem *)(hw_info->cldma_md_ao_base);
#endif
	md_ctrl->txq_active = 0;
	md_ctrl->rxq_active = 0;
	tasklet_init(&md_ctrl->cldma_rxq0_task, md_cldma_rxq0_tasklet, (unsigned long)md_ctrl);

	spin_lock_init(&md_ctrl->cldma_timeout_lock);
	for (i = 0; i < QUEUE_LEN(md_ctrl->txq); i++)
		md_cd_queue_struct_init(&md_ctrl->txq[i], md_ctrl->hif_id, OUT, i);
	for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++)
		md_cd_queue_struct_init(&md_ctrl->rxq[i], md_ctrl->hif_id, IN, i);

	md_ctrl->cldma_irq_id = hw_info->cldma_irq_id;
	md_ctrl->cldma_irq_worker =
	    alloc_workqueue("md%d_cldma_worker", WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_HIGHPRI, 1, md_id + 1);
...
int md_cd_late_init(unsigned char hif_id)
{
	...
	/* init ring buffers */
	md_ctrl->gpd_dmapool = dma_pool_create("cldma_request_DMA", ccci_md_get_dev_by_id(md_ctrl->md_id),
...
#endif
		cldma_tx_ring_init(md_ctrl, &md_ctrl->net_tx_ring[i]);
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "net_tx_ring %d: %p\n", i, &md_ctrl->net_tx_ring[i]);
	}
	for (i = 0; i < NET_RXQ_NUM; i++) {
		...
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "net_rx_ring %d: %p\n", i, &md_ctrl->net_rx_ring[i]);
	}
	for (i = 0; i < NORMAL_TXQ_NUM; i++) {
		...
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "normal_tx_ring %d: %p\n", i, &md_ctrl->normal_tx_ring[i]);
	}
	for (i = 0; i < NORMAL_RXQ_NUM; i++) {
		...
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "normal_rx_ring %d: %p\n", i, &md_ctrl->normal_rx_ring[i]);
	}
