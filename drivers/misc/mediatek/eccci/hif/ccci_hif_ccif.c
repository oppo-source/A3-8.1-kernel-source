...
static int c2k_ch_to_ccci_ch(int c2k_ch, int direction)
{
	...
	CCCI_ERROR_LOG(MD_SYS3, TAG, "%s:ERR cannot find mapped c2k ch ID(%d)\n", direction ? "TX" : "RX", c2k_ch);
...
static int ccci_ch_to_c2k_ch(int md_state, int ccci_ch, int direction)
{
	...
	for (i = 0; i < (sizeof(c2k_ports) / sizeof(struct c2k_port)); i++) {
		...
		if (ccci_channel_id == channel_map) {
			...
			return (md_state != EXCEPTION) ? c2k_ports[i].ch : c2k_ports[i].excp_ch;
		}
	}

	CCCI_ERROR_LOG(MD_SYS3, TAG, "%s:ERR cannot find mapped ccci ch ID(%d)\n", direction ? "TX" : "RX", ccci_ch);
...
//Yongyao.Song@PSW.NW.PWR.1053636, 2017/08/01, add for modem wake up source
	...
	CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "md_ccif_sram_rx_work:dk_pkg=%p, md_featrue=%p\n", dl_pkg, md_feature);
	pkg_size = sizeof(struct ccci_header) + sizeof(struct md_query_ap_feature);

	skb = ccci_alloc_skb(pkg_size, 1, 1);
	if (skb == NULL) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "md_ccif_sram_rx_work: alloc skb size=%d, failed\n", pkg_size);
		return;
	}
...
static int ccif_check_flow_ctrl(struct md_ccif_ctrl *md_ctrl, struct md_ccif_queue *queue, struct ccci_ringbuf *rx_buf)
{
	...
	if (is_busy < 0) {
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "ccif flow ctrl: check modem return %d\n", is_busy);
		return 0;
	}
	if (is_busy > 0) {
		...
		if (queue->resume_cnt < FLOW_CTRL_THRESHOLD &&
			buf_size <= rx_buf->rx_control.length / (2 << (queue->resume_cnt * 2))) {
			if (ccci_fsm_get_md_state(md_ctrl->md_id) == READY) {
				ret = ccci_port_send_msg_to_md(md_ctrl->md_id, CCCI_CONTROL_TX,
					C2K_FLOW_CTRL_MSG, queue->index, 0);
				if (ret < 0)
					CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "fail to resume md Q%d, ret0x%x\n",
						queue->index, ret);
				else {
					queue->resume_cnt++;
					CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "flow ctrl: resume Q%d, buf %d, cnt %d\n",
...
static void md_ccif_traffic_monitor_func(unsigned long data)
{
	...
	ccci_channel_dump_packet_counter(md_ctrl->md_id, &md_ctrl->traffic_info);
	/*pre_cnt for tx, pkt_cont for rx*/
	if (md_ctrl->md_id == MD_SYS3) {
		CCCI_REPEAT_LOG(md_ctrl->md_id, TAG,
			"traffic(AT): tx:[%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld\n",
			     CCCI_C2K_AT, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT],
			     CCCI_C2K_AT2, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT2],
			     CCCI_C2K_AT3, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT3],
			     CCCI_C2K_AT4, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT4],
			     CCCI_C2K_AT5, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT5],
			     CCCI_C2K_AT6, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT6],
			     CCCI_C2K_AT7, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT7],
			     CCCI_C2K_AT8, md_ctrl->traffic_info.logic_ch_pkt_pre_cnt[CCCI_C2K_AT8]);
		CCCI_REPEAT_LOG(md_ctrl->md_id, TAG,
			"traffic(AT): rx:[%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld\n",
			     CCCI_C2K_AT, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT],
			     CCCI_C2K_AT2, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT2],
			     CCCI_C2K_AT3, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT3],
			     CCCI_C2K_AT4, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT4],
			     CCCI_C2K_AT5, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT5],
			     CCCI_C2K_AT6, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT6],
			     CCCI_C2K_AT7, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT7],
			     CCCI_C2K_AT8, md_ctrl->traffic_info.logic_ch_pkt_cnt[CCCI_C2K_AT8]);
	}
...
static int ccif_rx_collect(struct md_ccif_queue *queue, int budget, int blocking, int *result)
{
	...
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)ccci_hif_get_by_id(queue->hif_id);
...
//Yongyao.Song@PSW.NW.PWR.1053636, 2017/08/01, add for modem wake up source
	...
	if (atomic_read(&queue->rx_on_going)) {
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d rx is on-going(%d)1\n",
	...
	while (1) {
		...
		if (pkg_size < 0) {
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d Rx:rbf readable ret=%d\n", queue->index, pkg_size);
			ret = 0;
			goto OUT;
		}
		...
		/*copy data into skb */
		spin_lock_irqsave(&queue->rx_lock, flags);
		ret = ccci_ringbuf_read(md_ctrl->md_id, rx_buf, data_ptr, pkg_size);
		spin_unlock_irqrestore(&queue->rx_lock, flags);
		if (unlikely(ret < 0)) {
			ccci_free_skb(skb);
			goto OUT;
		...
		if (md_ctrl->md_id == MD_SYS3) {
			...
			if ((ccci_h->channel == CCCI_CONTROL_RX) && (ccci_h->data[1] == C2K_HB_MSG)) {
				...
				CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "heart beat msg received\n");
			}
#endif
		...
		ret = ccci_port_recv_skb(md_ctrl->md_id, queue->hif_id, skb, NORMAL_DATA);

		if (ret >= 0 || ret == -CCCI_ERR_DROP_PACKET) {
			count++;
			ccci_md_check_rx_seq_num(md_ctrl->md_id, &md_ctrl->traffic_info, &ccci_hdr, queue->index);
			ccci_md_add_log_history(&md_ctrl->traffic_info, IN, (int)queue->index, &ccci_hdr,
				(ret >= 0 ? 0 : 1));
			ccci_channel_update_packet_counter(md_ctrl->traffic_info.logic_ch_pkt_cnt, &ccci_hdr);

			if (queue->debug_id) {
				CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "Q%d Rx recv req ret=%d\n", queue->index, ret);
				queue->debug_id = 0;
			}
			spin_lock_irqsave(&queue->rx_lock, flags);
			ccci_ringbuf_move_rpointer(md_ctrl->md_id, rx_buf, pkg_size);
			spin_unlock_irqrestore(&queue->rx_lock, flags);
			...
			if (queue->debug_id == 0) {
				queue->debug_id = 1;
				CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "Q%d Rx err, ret = 0x%x\n", queue->index, ret);
			}

			goto OUT;
		}
		if (count > budget) {
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d count > budget, exit now\n", queue->index);
			goto OUT;
		}
	}
 ...
 OUT:
	...
	*result = count;
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d rx %d pkg,ret=%d\n", queue->index, count, ret);
	spin_lock_irqsave(&queue->rx_lock, flags);
...
static void ccif_rx_work(struct work_struct *work)
{
	...
	if (ret == -EAGAIN) {
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%u queue again\n", queue->index);
		queue_work(queue->worker, &queue->qwork);
	} else {
		ccci_port_queue_status_notify(md_ctrl->md_id, queue->hif_id, queue->index, IN, RX_FLUSH);
	}
}
...
void ccif_polling_ready(unsigned char hif_id, int step)
{
	...
	while (cnt > 0) {
		if (md_ctrl->channel_id & (1 << step)) {
			...
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "poll RCHNUM %ld\n", md_ctrl->channel_id);
			return;
		}
		msleep(time_once);
		cnt--;
	}
	CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "poll EE HS timeout, RCHNUM %ld\n", md_ctrl->channel_id);
}
...
void md_ccif_sram_reset(unsigned char hif_id)
{
	...
	CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "md_ccif_sram_reset\n");
...
static void md_ccif_queue_dump(unsigned char hif_id)
{
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Dump md_ctrl->channel_id 0x%lx\n", md_ctrl->channel_id);
	ts = md_ctrl->traffic_info.latest_isr_time;
	nsec_rem = do_div(ts, NSEC_PER_SEC);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Dump CCIF latest isr %5llu.%06lu\n", ts, nsec_rem / 1000);

	for (idx = 0; idx < QUEUE_NUM; idx++) {
		CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Q%d TX: w=%d, r=%d, len=%d\n",
		...
		CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Q%d RX: w=%d, r=%d, len=%d\n",
		...
		nsec_rem = do_div(ts, NSEC_PER_SEC);
		CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Q%d RX: last isr %5llu.%06lu\n", idx, ts, nsec_rem / 1000);
		ts = md_ctrl->traffic_info.latest_q_rx_time[idx];
		nsec_rem = do_div(ts, NSEC_PER_SEC);
		CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Q%d RX: last wq  %5llu.%06lu\n", idx, ts, nsec_rem / 1000);
	}
	ccci_md_dump_log_history(md_ctrl->md_id, &md_ctrl->traffic_info, 1, QUEUE_NUM, QUEUE_NUM);
}
...
void md_ccif_reset_queue(unsigned char hif_id, unsigned char for_start)
{
	...
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)ccci_hif_get_by_id(hif_id);
	unsigned long flags;
	int ccif_id = md_ctrl->md_id == MD_SYS1 ? AP_MD1_CCIF : AP_MD3_CCIF;
	...
	for (i = 0; i < QUEUE_NUM; ++i) {
		...
		ccci_ringbuf_reset(md_ctrl->md_id, md_ctrl->rxq[i].ringbuf, 0);
		...
		ccci_ringbuf_reset(md_ctrl->md_id, md_ctrl->txq[i].ringbuf, 1);
...
{
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "ccif_irq_tasklet1: ch %lx\n", md_ctrl->channel_id);
	if (md_ctrl->channel_id & CCIF_HW_CH_RX_RESERVED) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "Interrupt from reserved ccif ch(%ld)\n", md_ctrl->channel_id);
		md_ctrl->channel_id &= ~CCIF_HW_CH_RX_RESERVED;
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "After cleared reserved ccif ch(%ld)\n", md_ctrl->channel_id);
	}
	if (md_ctrl->channel_id & (1 << D2H_EXCEPTION_INIT)) {
		clear_bit(D2H_EXCEPTION_INIT, &md_ctrl->channel_id);
		ccci_fsm_recv_md_interrupt(md_ctrl->md_id, MD_IRQ_CCIF_EX);
	}

	if (md_ctrl->channel_id & (1 << AP_MD_SEQ_ERROR)) {
		clear_bit(AP_MD_SEQ_ERROR, &md_ctrl->channel_id);
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "MD check seq fail\n");
		ccci_md_dump_info(md_ctrl->md_id, DUMP_FLAG_CCIF, NULL, 0);
	}
	if (md_ctrl->channel_id & (1 << (D2H_SRAM))) {
		clear_bit(D2H_SRAM, &md_ctrl->channel_id);
		schedule_work(&md_ctrl->ccif_sram_work);
	}
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "ccif_irq_tasklet2: ch %ld\n", md_ctrl->channel_id);
}
	...
	if (md_ctrl->channel_id & (1 << AP_MD_CCB_WAKEUP)) {
		...
		if (atomic_cmpxchg(&md_ctrl->wakeup_src, 1, 0) == 1) {
			...
			CCCI_NOTICE_LOG(md_ctrl->md_id, TAG, "CCIF_MD wakeup source:(CCB)(%u)\n",
				md_ctrl->wakeup_count);
			//#ifdef VENDOR_EDIT
			//Yuanfei.Liu@PSW.NW.PWR.1257900.1284205, 2018/02/23, add for modem log wake up source CCB
			modem_wakeup_src_count[0]++;
			//#endif /* VENDOR_EDIT */
		}
	...
	for (i = 0; i < QUEUE_NUM; i++) {
		if (md_ctrl->channel_id & (1 << (i + D2H_RINGQ0))) {
			...
			if (atomic_read(&md_ctrl->rxq[i].rx_on_going)) {
				CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d rx is on-going(%d)2\n",
					     md_ctrl->rxq[i].index,
...
static inline void md_ccif_queue_struct_init(struct md_ccif_queue *queue, unsigned char hif_id,
...
static int md_ccif_op_write_room(unsigned char hif_id, unsigned char qno)
{
	...
	return ccci_ringbuf_writeable(md_ctrl->md_id, md_ctrl->txq[qno].ringbuf, 0);
}
			       ...
			       struct sk_buff *skb, int skb_from_pool, int blocking)
{
 ...
 retry:
	...
	if (ccci_ringbuf_writeable(md_ctrl->md_id, queue->ringbuf, skb->len) > 0) {
		if (ccci_h->channel == CCCI_C2K_LB_DL) {
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "Q%d Tx lb_dl\n", queue->index);
			c2k_mem_dump(skb->data, skb->len);
		}
		ccci_md_inc_tx_seq_num(md_ctrl->md_id, &md_ctrl->traffic_info, ccci_h);
		...
		if (md_ctrl->md_id == MD_SYS3) {
			 ...
			 */
			ccci_to_c2k_ch = ccci_ch_to_c2k_ch(ccci_fsm_get_md_state(md_ctrl->md_id), ccci_h->channel, OUT);
			...
			if (ccci_h->data[1] == C2K_HB_MSG)
				CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "hb: 0x%x\n", ccci_h->channel);
		}
		/*copy skb to ringbuf */
		ret = ccci_ringbuf_write(md_ctrl->md_id, queue->ringbuf, skb->data, skb->len);
		if (ret != skb->len)
			CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "TX:ERR rbf write: ret(%d)!=req(%d)\n", ret, skb->len);
		...
		if (likely(md_cap & MODEM_CAP_TXBUSY_STOP) && md_flow_ctrl > 0) {
			ccif_set_busy_queue(md_ctrl, qno);
			/*double check tx buffer after set busy bit. it is to avoid tx buffer is empty now*/
			if (unlikely(ccci_ringbuf_writeable(md_ctrl->md_id, queue->ringbuf, skb->len) > 0)) {
				ccif_clear_busy_queue(md_ctrl, qno);
				spin_unlock_irqrestore(&queue->tx_lock, flags);
				goto retry;
			} else {
				CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "flow ctrl: TX busy on Q%d\n", queue->index);
		...
		if (blocking) {
			if (md_flow_ctrl > 0) {
				CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "flow ctrl: Q%d is blocking, skb->len = %d\n",
					queue->index, skb->len);
			...
			if (per_md_data->data_usb_bypass)
				...
				return -EBUSY;
			CCCI_DEBUG_LOG(md_ctrl->md_id, TAG, "tx fail on q%d\n", qno);
		}
	}
	return 0;
}
...
static void md_ccif_dump_queue_history(unsigned char hif_id, unsigned int qno)
{
	...
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Dump md_ctrl->channel_id 0x%lx\n", md_ctrl->channel_id);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Dump CCIF Queue%d Control\n", qno);
	CCCI_MEM_LOG_TAG(md_ctrl->md_id, TAG, "Q%d TX: w=%d, r=%d, len=%d\n",
...
static int md_ccif_stop_queue(unsigned char hif_id, unsigned char qno, DIRECTION dir)
{
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)ccci_hif_get_by_id(hif_id);
...
static int md_ccif_start_queue(unsigned char hif_id, unsigned char qno, DIRECTION dir)
{
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)ccci_hif_get_by_id(hif_id);
	struct md_ccif_queue *queue = NULL;
	unsigned long flags;

	if (dir == OUT && likely(ccci_md_get_cap_by_id(md_ctrl->md_id) & MODEM_CAP_TXBUSY_STOP && (qno < QUEUE_NUM))) {
		queue = &md_ctrl->txq[qno];
		spin_lock_irqsave(&queue->tx_lock, flags);
		ccif_wake_up_tx_queue(md_ctrl, qno);
		/*special for net queue*/
		ccci_hif_queue_status_notify(md_ctrl->md_id, hif_id, qno, OUT, TX_IRQ);
		spin_unlock_irqrestore(&queue->tx_lock, flags);
	}
	return 0;
}
...
int md_ccif_exp_ring_buf_init(struct md_ccif_ctrl *md_ctrl)
{
	...
	ccism = ccci_md_get_smem_by_user_id(md_ctrl->md_id, SMEM_USER_CCISM_MCU_EXP);

	buf = (unsigned char *)ccism->base_ap_view_vir;
	for (i = 0; i < QUEUE_NUM; i++) {
		bufsize = CCCI_RINGBUF_CTL_LEN + rx_exp_buffer_size[i] + tx_exp_buffer_size[i];
		ringbuf =
		    ccci_create_ringbuf(md_ctrl->md_id, buf, bufsize, rx_exp_buffer_size[i], tx_exp_buffer_size[i]);
		if (ringbuf == NULL) {
			CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "ccci_create_ringbuf %d failed\n", i);
			return -1;
		}
		/*rx */
...
int md_ccif_ring_buf_init(unsigned char hif_id)
{
	...
	ccism = ccci_md_get_smem_by_user_id(md_ctrl->md_id, SMEM_USER_CCISM_MCU);
	...
	for (i = 0; i < QUEUE_NUM; i++) {
		...
		ringbuf =
		    ccci_create_ringbuf(md_ctrl->md_id, buf, bufsize, rx_queue_buffer_size[i], tx_queue_buffer_size[i]);
		if (ringbuf == NULL) {
			CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "ccci_create_ringbuf %d failed\n", i);
			return -1;
		}
	...
	if (ccci_md_get_cap_by_id(md_ctrl->md_id) & MODEM_CAP_TXBUSY_STOP) {
		md_ctrl->flow_ctrl = (struct ccif_flow_control *)(ccism->base_ap_view_vir + md_ctrl->total_smem_size);
...
static int md_ccif_op_dump_status(unsigned char hif_id, MODEM_DUMP_FLAG flag, int length)
{
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)ccci_hif_get_by_id(hif_id);
	...
	if (flag & DUMP_FLAG_IRQ_STATUS) {
		CCCI_NORMAL_LOG(md_ctrl->md_id, TAG, "Dump AP CCIF IRQ status\n");
...
static void ccif_hif_hw_init(struct md_ccif_ctrl *md_ctrl)
{
	...
	CCCI_INIT_LOG(md_ctrl->md_id, TAG, "CCIF_AP IRQ(%d) flag 0x%lx\n", md_ctrl->ccif_irq_id, ccif_irq_flags);

	ret = request_irq(md_ctrl->ccif_irq_id, md_ccif_isr, ccif_irq_flags, "CCIF_AP_DATA", md_ctrl);
	if (ret) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "request CCIF_AP_DATA IRQ(%d) error %d\n",
...
int ccci_ccif_hif_init(unsigned char hif_id, unsigned char md_id)
{
	...
	/*init queue */
	for (i = 0; i < QUEUE_NUM; i++) {
		md_ccif_queue_struct_init(&md_ctrl->txq[i], md_ctrl->hif_id, OUT, i);
		md_ccif_queue_struct_init(&md_ctrl->rxq[i], md_ctrl->hif_id, IN, i);
	}
...
void *ccif_hif_fill_rt_header(unsigned char hif_id, int packet_size, unsigned int tx_ch, unsigned int txqno)
{
	...
	*((u32 *)&ccci_h_bk + 2) = ccif_read32((u32 *) ccci_h + 2, 0);
	...
	ccci_md_add_log_history(&md_ctrl->traffic_info, OUT, (int)txqno, &ccci_h_bk, 0);
