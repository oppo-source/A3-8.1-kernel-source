...
/*
* Ling.Guo@PSW.MM.Display.LCD.Stability, 2018/01/05,
...
static const char *session_mode_spy_for_align(unsigned int mode)
{
	switch (mode) {
...
void debug_print_power_mode_check(enum mtkfb_power_mode prev, enum mtkfb_power_mode cur)
{
	if (primary_display_check_power_mode() != cur)
		DISPPR_ERROR("AOD check error: fail to set FB power mode %s to %s(but now %s)\n",
...
static void add_fps_info_for_debug(void)
{
	...
	if (layer_show[show_layer_fps] == 1) {
		snprintf(p, sizeof(disp_tmp), "fps:%2lld.%01lld ",
			fps_info_debug.total_fps_high, fps_info_debug.total_fps_low);
	...
	for (i = 0; i < show_layer_fps; i++) {
		if (layer_show[i] == 1) {
			...
			fps_info_debug.layer_fps_high[i], fps_info_debug.layer_fps_low[i]);
...
static void add_layer_info_for_debug(void)
{
	...
	memset(disp_tmp, 0, sizeof(disp_tmp));
	if (hrt_show_flag == 1) {
		snprintf(p, sizeof(disp_tmp), "hrt:%01d.%01d ", hrt_high, hrt_low);
		j = strlen(p);
		p = p + j;
	}
...
void primary_show_info_to_screen(struct disp_ddp_path_config *cfg)
{
	...
	if (path_mode_flag == 1)
		screen_logger_add_message("sess_mode", MESSAGE_REPLACE,
			(char *)session_mode_spy_for_align(pgc->session_mode));
	else
...
static int fps_ctx_get_fps(struct fps_ctx_t *fps_ctx, unsigned int *fps, int *stable)
{
	...
	*stable = 1;
	...
	mmprofile_log_ex(ddp_mmp_get_events()->fps_get, MMPROFILE_FLAG_PULSE, *fps, *stable);
	mutex_unlock(&fps_ctx->lock);
	return 0;
}
...
static int fps_ext_ctx_init(struct fps_ext_ctx_t *fps_ctx, unsigned int ms)
{
	...
	if (fps_ctx == NULL) {
		DISPPR_ERROR("%s:%d, fps_cxt is null\n", __func__, __LINE__);
		return -1;
	}
	if (ms > INTERVAL_MS) {
		DISPPR_ERROR("%s:%d, interval is too big:%d ms, max:%d ms\n", __func__, __LINE__, ms, INTERVAL_MS);
		return -1;
	}
	if (ms == 0) {
		DISPPR_ERROR("%s:%d, interval is equal to zero\n", __func__, __LINE__);
		return -1;
	}
...
static int fps_ext_ctx_get_fps(struct fps_ext_ctx_t *fps_ctx, unsigned int *fps, int *stable)
{
	...
	if (fps == NULL) {
		DISPPR_ERROR("%s:%d, fps is null\n", __func__, __LINE__);
		return -1;
	}
	if (stable == NULL) {
		DISPPR_ERROR("%s:%d, stable is null\n", __func__, __LINE__);
		return -1;
	}
...
int primary_display_get_debug_state(char *stringbuf, int buf_len)
{
	...
	len += scnprintf(stringbuf + len, buf_len - len,
		      ...
		      session_mode_spy(pgc->session_mode), pgc->vsync_drop);
	len += scnprintf(stringbuf + len, buf_len - len,
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if (disp_helper_get_option(DISP_OPT_ARR_PHASE_1)) {
			...
			dpmgr_path_ioctl(pgc->dpmgr_handle, pgc->cmdq_handle_trigger,
					 DDP_DSI_PORCH_ADDR, (void *)(dsi_vfp_addr));

			if (dsi_vfp_addr[0]) {
				cmdqRecBackupWriteRegisterFromSlot(pgc->cmdq_handle_trigger,
								   pgc->dsi_vfp_line, 0,
...
void _cmdq_start_trigger_loop(void)
{
	...
	/* this should be called only once because trigger loop will never stop */
	...
	if (state == FRM_CONFIG) {
		...
		for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
			if ((abs(addr - frm_update_sequence[i].mva) <= frm_update_sequence[i].max_offset) &&
				...
				mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_rdma_irq,
					       MMPROFILE_FLAG_PULSE, frm_update_sequence[i].mva,
		...
		for (i = 0; i < FRM_UPDATE_SEQ_CACHE_NUM; i++) {
			if (frm_update_sequence[i].state == FRM_START) {
				...
				mmprofile_log_ex(ddp_mmp_get_events()->primary_seq_release,
					       MMPROFILE_FLAG_PULSE, frm_update_sequence[i].mva,
					       frm_update_sequence[i].seq);
			}
		}
...
static void directlink_path_add_memory(struct WDMA_CONFIG_STRUCT *p_wdma, enum DISP_MODULE_ENUM after_engine)
{
	...
	if (ret) {
		DISPPR_ERROR("dl_to_dc capture:Fail to create cmdq handle\n");
		ret = -1;
		goto out;
	}
	...
	if (ret) {
		DISPPR_ERROR("dl_to_dc capture:Fail to create cmdq wait handle\n");
		ret = -1;
		goto out;
	}
...
static int sec_buf_ion_alloc(int buf_size)
{
	...
	if (!ion_client) {
		DISPPR_ERROR("create ion client failed!\n");
		return -1;
	}
	...
	if (IS_ERR_OR_NULL(sec_ion_handle)) {
		DISPPR_ERROR("Fatal Error, ion_alloc for size %d failed\n", buf_size);
	...
	if (ion_kernel_ioctl(ion_client, ION_CMD_MULTIMEDIA_SEC, (unsigned long)&mm_data) < 0) {
		DISPPR_ERROR("ion_test_drv: Config buffer failed.\n");
		ion_free(ion_client, sec_ion_handle);
		ion_client_destroy(ion_client);
		return -1;
	}
	...
	if (ion_phys(ion_client, sec_ion_handle, &phy_addr, (size_t *)&mva_size)) {
		DISPPR_ERROR("can't query ion buffer physical address!\n");
		ion_free(ion_client, sec_ion_handle);
		ion_client_destroy(ion_client);
		return -1;
	}
	...
	if (sec_hnd == 0) {
		DISPPR_ERROR("Fatal Error, get mva failed\n");
		ion_free(ion_client, sec_ion_handle);
		ion_client_destroy(ion_client);
		return -1;
	}
...
static int init_sec_buf(void)
{
	...
	if (ret) {
		DISPPR_ERROR("sec_buf_ion_alloc failed!\n");
		return -1;
	}
...
static int _DL_switch_to_DC_fast(int block)
{
	...
	if (path_mode_flag == 1) {
		rdma_buffer_flag = 1;
		screen_logger_add_message("sess_mode", MESSAGE_REPLACE,
			(char *)session_mode_spy_for_align(DISP_SESSION_DECOUPLE_MODE));
		dynamic_debug_msg_print(mva, rdma_config.width, rdma_config.height,
				rdma_config.pitch,
				UFMT_GET_Bpp(rdma_config.inputFormat));
		rdma_buffer_flag = 0;
...
static struct disp_internal_buffer_info *allocat_decouple_buffer(int size)
{
	...
	if (buf_info) {
		...
		if (buffer_va == NULL) {
			DISPPR_ERROR("ion_map_kernrl failed\n");
			ion_free(client, handle);
			ion_client_destroy(client);
			kfree(buf_info);
			return NULL;
		...
		DISPPR_ERROR("Fatal error, kzalloc internal buffer info failed!!\n");
		kfree(buf_info);
		return NULL;
	}
#endif
...
static int _convert_disp_input_to_ovl(struct OVL_CONFIG_STRUCT *dst, struct disp_input_config *src)
{
	...
	if (src->buffer_source == DISP_BUFFER_ALPHA) {
		...
		DISPPR_ERROR("unknown source = %d", src->buffer_source);
...
int primary_display_change_lcm_resolution(unsigned int width, unsigned int height)
{
	...
	if (width > pgc->plcm->params->width ||
		...
		DISPPR_ERROR("Invalid resolution: %dx%d\n", width, height);
		return -1;
	}

	if (primary_display_is_video_mode()) {
		DISPPR_ERROR("Warning!!!Video Mode can't support multiple resolution!\n");
		return -1;
	}
...
static int _Interface_fence_release_callback(unsigned long userdata)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if ((status) != 0x10001) {
			/* dither is not idle !! */
			DISPPR_ERROR("disp dither status error!! stat=0x%x\n", status);
...
static int _decouple_update_rdma_config_nolock(void)
{
	...
	if (primary_display_is_decouple_mode()) {
		...
		if (primary_get_state() != DISP_ALIVE) {
			_rdma_update_callback(interface_fence > 1 ? interface_fence - 1 : 0);
			return -1;
		}
...
#ifdef VENDOR_EDIT
/*
* Ling.Guo@PSW.MM.Display.LCD.Stability, 2018/01/19,
* add for round corner
*/
void add_round_corner_layers(struct disp_ddp_path_config *cfg, unsigned int w, unsigned int h,
	unsigned int h_bot, unsigned int pitch, unsigned long mva_top, unsigned long mva_bot,
...
static int _decouple_update_ovl1_config_nolock(void)
{
	...
	if (lcm_corner_en) {
		if (disp_helper_get_option(DISP_OPT_ROUND_CORNER))
			add_round_corner_layers(pconfig, primary_display_get_width(),
...
static int _ovl_fence_release_callback(unsigned long userdata)
{
	...
	for (i = 0; i < PRIMARY_SESSION_INPUT_LAYER_COUNT; i++) {
		...
		mmprofile_log_ex(ddp_mmp_get_events()->primary_ovl_fence_release,
...
static int primary_display_remove_output(void *callback, unsigned int userdata)
{
	...
	if (ret == 0) {
		...
		if (ret == 0) {
			...
			DISPPR_ERROR("fail to create  wait handle\n");
		}
		cmdqRecReset(cmdq_handle);
		...
		ret = -1;
		DISPPR_ERROR("fail to remove memout out\n");
	}
	return ret;
}
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps, int is_lcm_inited)
{
	...
	if (unlikely(lcm_param == NULL)) {
		DISPPR_ERROR("get lcm params FAILED\n");
		ret = DISP_STATUS_ERROR;
		goto done;
	}
	...
	if (use_cmdq) {
		...
		if (ret) {
			DISPPR_ERROR("cmdqCoreRegisterCB failed, ret=%d\n", ret);
			ret = DISP_STATUS_ERROR;
			goto done;
		}
		...
		if (ret) {
			DISPPR_ERROR("cmdqRecCreate FAIL, ret=%d\n", ret);
			ret = DISP_STATUS_ERROR;
			goto done;
		}
	} else {
	...
	if (IS_ERR(init_decouple_buffer_thread))
		DISPPR_ERROR("kthread_run init_decouple_buffer_thread err = %d",
...
int _display_set_lcm_refresh_rate(int fps)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_switch_fps, MMPROFILE_FLAG_START, fps, 0);
...
int primary_display_suspend(void)
{
	...
	if (primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		#ifndef VENDOR_EDIT
		/*
		* Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/01/16,
		* add for Aod feature
		*/
...
#ifdef VENDOR_EDIT
/* Xinqin.Yang@Cam.Tuning.Display, 2018/01/22, add for multi-lcms */
...
static int check_switch_lcm_mode_for_debug(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume, MMPROFILE_FLAG_PULSE, 0, lcm_mode_status);
	lcm_mode_status = 0;
...
int primary_display_resume(void)
{
	...
	*/
...

	{
		...
		if (primary_display_is_decouple_mode()) {
			if (pgc->ovl2mem_path_handle)
				...
				DISPPR_ERROR("in decouple_mode but no ovl2mem_path_handle\n");
		}
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		DISPPR_ERROR("[POWER]Fatal error, we didn't start display path but it's already busy\n");
		ret = -1;
		/* goto done; */
	}
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		...
		DISPPR_ERROR("[POWER]Fatal error, we didn't trigger display path but it's already busy\n");
		ret = -1;
		/* goto done; */
	}
...
int primary_display_start(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		DISPPR_ERROR("Fatal error, we didn't trigger display path but it's already busy\n");
		ret = -1;
		goto done;
	}
...
int primary_display_stop(void)
{
	...
	if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
		DISPPR_ERROR("stop display path failed, still busy\n");
		ret = -1;
		goto done;
	}
...
static int primary_display_trigger_nolock(int blocking, void *callback, int need_merge)
{
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE ||
		...
		_trigger_ovl_to_memory(pgc->ovl2mem_path_handle, pgc->cmdq_handle_ovl1to2_config,
				       (CmdqAsyncFlushCB)decouple_trigger_callback, DISP_SESSION_DECOUPLE_MODE);
...
static int decouple_trigger_worker_thread(void *data)
{
	...
	while (1) {
		...
		_primary_path_unlock(__func__);
		if (kthread_should_stop()) {
			DISPPR_ERROR("error: stop %s as demond\n", __func__);
			break;
		}
	}
	return 0;
...
static int primary_frame_cfg_output(struct disp_frame_cfg_t *cfg)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPPR_ERROR("mem out is already slept or mode wrong(%d)\n", pgc->session_mode);
		goto done;
	}

	if (!primary_display_is_mirror_mode()) {
		DISPPR_ERROR("should not config output if not mirror mode!!\n");
		goto done;
	}
			  ...
			  struct cmdqRecStruct *cmdq_handle, int is_locked)
{
	...
	if (has_sec_layer != primary_is_sec()) {
		mmprofile_log_ex(ddp_mmp_get_events()->sec, MMPROFILE_FLAG_PULSE, has_sec_layer, 0);
		/* sec/nonsec switch */
		...
		mmprofile_log_ex(ddp_mmp_get_events()->sec, MMPROFILE_FLAG_PULSE, has_sec_layer, 1);

		if (has_sec_layer) {
			/* switch nonsec --> sec */
			svp_state = SVP_IN_POINT;
			disp_enter_svp(svp_state);
			mmprofile_log_ex(ddp_mmp_get_events()->sec, MMPROFILE_FLAG_START, 0, 0);
		} else {
			/* switch sec --> nonsec */
			svp_state = SVP_2_NOMAL;
			svp_sum = 0;
			mmprofile_log_ex(ddp_mmp_get_events()->sec, MMPROFILE_FLAG_END, 0, 0);
		}
	}
			  ...
			  int *bypass_layer_id, int dc_request)
{
	...
	*/
	#ifdef CONFIG_MTK_ROUND_CORNER_SUPPORT
	if (lcm_corner_en) {
		if (w != primary_display_get_width() || h != primary_display_get_height() ||
			ufmt_get_Bpp(data_config->ovl_config[*bypass_layer_id].fmt) != 4)
			return 0;
	}
	#endif
	...
	if (vram_buf_size > dal_buf_size + frame_buf_size) {
		...
		*pitch = primary_display_get_width();
		...
		if (full_content) {
			...
			bt = frame_buf_mva + vram_buf_size - corner_size - bt_size;
		} else {
...
static struct rc_buf *get_rc_buf(void)
{
	...
	ret = get_round_corner_param(NULL, NULL, &lt_pa, &lb_pa, &rc.pitch, &rc.h, &rc.h);
	if (ret)
		return NULL;
			     ...
			     struct cmdqRecStruct *cmdq_handle)
{
	...
	int dc_request, old_mode, new_mode;
	struct disp_rect total_dirty_roi = {0, 0, 0, 0};
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (layer_en_num_flag == 1 && ovl_cfg->layer_en == 1 && i < (TOTAL_OVL_LAYER_NUM - 1))
			layer_en_num++;
	...
	if ((hrt_level - dvfs_last_ovl_req) > 0 && (!primary_display_is_decouple_mode()))
	...
	if (cfg->setter != SESSION_USER_INVALID) {
		...
		if (old_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			if (bypass) {
				...
				DISPWARN("hrt request DC mode without a rdma mode frame!\n");
	...
	if (primary_display_is_decouple_mode() &&
		...
		primary_display_is_directlink_mode()) {
		primary_display_request_dvfs_perf(MMDVFS_SCEN_DISP, HRT_LEVEL_LEVEL2);
	...
	if (lcm_corner_en) {
		if (disp_helper_get_option(DISP_OPT_ROUND_CORNER) &&
			...
			add_round_corner_layers(data_config, primary_display_get_width(),
	...
	if (get_show_info_to_screen_flg()) {
		...
		if (create_thread_flag == 0 && fps_show_flag == 1) {
			...
			primary_fps_monitor_task = kthread_create(_primary_monitor_fps_thread, NULL, "fps_monitor");
...
int primary_display_config_input_multiple(struct disp_session_input_config *session_input)
{
	...
	if (sizeof(session_input->config) != sizeof(frame_cfg->input_cfg)) {
		DISPPR_ERROR("session input config size not equal frame config size\n");
		return -1;
	}
...
int primary_display_frame_cfg(struct disp_frame_cfg_t *cfg)
{
	...
	if (trigger_event) {
		...
		unsigned int proc_name = (current->comm[0] << 24) | (current->comm[1] << 16) |
					...
					struct cmdqRecStruct *handle, int block)
{
	...
	if (path_mode_flag == 1)
		screen_logger_add_message("sess_mode", MESSAGE_REPLACE, (char *)session_mode_spy_for_align(sess_mode));
...
int primary_display_get_info(struct disp_session_info *info)
{
	...
	if (lcm_param == NULL) {
		DISPPR_ERROR("lcm_param is null\n");
		return -1;
	}
	...
	if (disp_helper_get_option(DISP_OPT_FPS_EXT))
		fps_ext_ctx_get_fps(&primary_fps_ext_ctx, &dispif_info->updateFPS,
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		#ifdef VENDOR_EDIT
		/*Guoqiang.jiang@MM.Display.LCD.Machine.1234141, 2018/03/03, add for auto backlight adjust block.*/
		_cmdq_flush_config_handle_mira(cmdq_handle_backlight, 0);
		#else
...
int primary_display_setbacklight(unsigned int level)
{
	...
	*/
	if (primary_display_get_power_mode_nolock() == DOZE
		|| primary_display_get_power_mode_nolock() == DOZE_SUSPEND) {
		if (is_project(OPPO_17197)) {
			DISPMSG("OLED need skip set backlight on DOZE\n");
			return 0;
		}
	}
	#endif /* VENDOR_EDIT */
...
int _set_cabc_mode_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd, MMPROFILE_FLAG_PULSE, 1, 2);
		cmdqRecReset(cmdq_handle_lcm_cmd);
		/* Shizeke@PSW.MultiMedia.Display.LCD.Machine. 2018/8/15, Add for cabc send cmd wait for frame done in realme 18611 */
...
/*
* Ling.Guo@PSW.MM.Display.LCD.Stability, 2018/01/16,
...
int _set_hbm_mode_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if((!strcmp(pgc->plcm->drv->name,
				"oppo_samsung_ams628nw_1080p_dsi_cmd"))
			|| (!strcmp(pgc->plcm->drv->name,
				"oppo_samsung_ams628nw_lsi_1080p_dsi_cmd"))){
...
/*
* Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/01/26,
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, enum UNIFIED_COLOR_FMT ufmt)
{
	...
	if (ion_display_client == NULL) {
		DISPPR_ERROR("primary capture:Fail to create ion\n");
		ret = -1;
		goto out;
	}
...
int fbconfig_get_esd_check_test(UINT32 dsi_id, UINT32 cmd, UINT8 *buffer, UINT32 num)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->esd_check_t, MMPROFILE_FLAG_START, cmd, -1);
...
int Panel_Master_dsi_config_entry(const char *name, void *config_value)
{
	...
	lcm_param = disp_lcm_get_params(pgc->plcm);
	if (pgc->state == DISP_SLEPT) {
		DISPPR_ERROR("[Pmaster]Panel_Master: primary display path is slept??\n");
		goto done;
	}
...
int primary_display_switch_dst_mode(int mode)
{
	...
	if (lcm_cmd == NULL) {
		...
		DISPCHECK("[primary_display_switch_dst_mode]get lcm cmd fail primary_display_cur_dst_mode=%d mode=%d\n",
			  primary_display_cur_dst_mode, mode);
		goto done;
	}
	...
	if (dpmgr_path_ioctl(pgc->dpmgr_handle, pgc->cmdq_handle_config, DDP_SWITCH_DSI_MODE, lcm_cmd) != 0) {
		...
		/* DISPPR_ERROR("[C2V]switch dsi mode fail, return directly\n"); */
		ret = -1;
	}
...
int display_enter_tui(void)
{
	...
	if (primary_get_state() != DISP_ALIVE) {
		DISPPR_ERROR("Can't enter tui: current_stat=%d is not alive\n", primary_get_state());
		goto err0;
	}
	...
	if (primary_display_is_mirror_mode()) {
		DISPPR_ERROR("Can't enter tui: current_mode=%s\n", session_mode_spy(pgc->session_mode));
		goto err1;
	}
