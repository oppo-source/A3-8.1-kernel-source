...
static int mtkfb1_blank(int blank_mode, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
		/*parameter 0 is to all external display should be resumed, 0x20003 is dual lcm session*/
	...
	case FB_BLANK_POWERDOWN:
		/*parameter 0 is to all external display should be suspend, 0x20003 is dual lcm session*/
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if (!session_input) {
		DDPPR_ERR("session input allocat fail\n");
		ASSERT(0);
		return -1;
	}
	...
	case 32:
		input->src_fmt =
		    (var->blue.offset == 0) ? DISP_FORMAT_BGRA8888 : DISP_FORMAT_RGBX8888;
...
static int mtkfb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case MTKFB_POWEROFF:
		{
			MTKFB_FUNC();
			if (primary_display_is_sleepd()) {
				DDPPR_ERR("[FB Driver] is still in MTKFB_POWEROFF!!!\n");
				return r;
			}
	...
	case MTKFB_SET_OVERLAY_LAYER:
		{		/* no function */
			...
			if (copy_from_user(layerInfo, (void __user *)arg, sizeof(*layerInfo))) {
				...
				if (primary_display_is_sleepd()) {
					DDPPR_ERR("[FB] error, set overlay in early suspend ,skip!\n");
					kfree(layerInfo);
	...
	case MTKFB_ERROR_INDEX_UPDATE_TIMEOUT:
		{
			DDPPR_ERR("[DDP] mtkfb_ioctl():MTKFB_ERROR_INDEX_UPDATE_TIMEOUT\n");
...
static int mtkfb_compat_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_MTKFB_SET_OVERLAY_LAYER:
	{
		...
		if (copy_from_user(compat_layerInfo, (void __user *)arg, sizeof(*compat_layerInfo))) {
			...
			/* in early suspend mode ,will not update buffer index, info SF by return value */
			if (primary_display_is_sleepd()) {
				DDPPR_ERR("[FB Driver] error, set overlay in early suspend ,skip!\n");
...
static int mtkfb_fbinfo_init(struct fb_info *info)
{
	...
	if (r != 0)
		DDPPR_ERR("unable to allocate color map memory\n");
...
*
	...
	{
		_parse_tag_videolfb();
		DISPDBG("%s, %s\n", __func__, ext_mtkfb_lcm_name);
		return ext_mtkfb_lcm_name;
	}
	#endif
*/
...
#ifdef VENDOR_EDIT
/* Guoqiang.jiang@MM.Display.LCD.Machine, 2018/03/13, add for backlight IC KTD3136 */
/* Shizeke@MM.Display.LCD.Machine, 2018/8/15, add for backlight IC KTD3136 in realme 18611 */
void get_backlight_ic(void) {
	if (is_project(OPPO_17175) || is_project(OPPO_18311) || is_project(OPPO_18011) || is_project(OPPO_18611)) {
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	if (r) {
		DDPPR_ERR("mtkfb_fbinfo_init fail, r = %d\n", r);
		goto cleanup;
	}
	init_state++;		/* 4 */
	...
	if (r) {
		DDPPR_ERR("mtkfb_register_sysfs fail, r = %d\n", r);
		goto cleanup;
	}
	init_state++;		/* 5 */
