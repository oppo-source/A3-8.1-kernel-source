...
int disp_create_session(struct disp_session_config *config)
{
	...
	if (idx != -1) {
		...
		DISPPR_ERROR("Invalid session creation request\n");
		ret = -1;
	}
done:
...
static int _get_max_layer(unsigned int session_id)
{
	...
	DISPPR_ERROR("session_id is wrong!!\n");
	return 0;
}
...
static int disp_input_get_dirty_roi(struct disp_frame_cfg_t *cfg)
{
	...
	for (i = 0; i < cfg->input_layer_num; i++) {
		...
		if (copy_from_user(addr, cfg->input_cfg[i].dirty_roi_addr, size)) {
			DISPPR_ERROR("[drity roi]: copy_from_user failed! line:%d\n", __LINE__);
			DISPPR_ERROR("to=%p, from=%p, size=0x%lx\n", addr, cfg->input_cfg[i].dirty_roi_addr, size);
			kfree(addr);
			goto layer_err;
...
int disp_mgr_get_session_info(struct disp_session_info *info)
{
	...
	if (DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY) {
		...
		DISPPR_ERROR("session type is wrong:0x%08x\n", session_id);
		return -1;
	}
...
int _ioctl_get_display_caps(unsigned long arg)
{
	...
	if (disp_helper_get_option(DISP_OPT_AOD) == 0) /* add read HW config to decide enable AOD or not */
...
int _ioctl_set_vsync(unsigned long arg)
{
	...
	if ((fps < 50) || (fps > 60)) {
		DISPPR_ERROR("_ioctl_set_vsync fps setting is out of range, fps=%d\n", fps);
		return  -EFAULT;
	}
...
int set_session_mode(struct disp_session_config *config_info, int force)
{
	...
	if (config_info->mode == DISP_SESSION_DIRECT_LINK_MIRROR_MODE ||
		...
		if (has_memory_session)
			...
			DISPPR_ERROR("[FB]: session(0x%x) swith mode(%d) fail\n",
...
/* Xinqin.Yang@Cam.Tuning.Display, 2018/01/22, add for multi-lcms */
	...
	default:
	{
		DISPPR_ERROR("[session]ioctl not supported, 0x%08x\n", cmd);
	}
	}
...
static long mtk_disp_mgr_compat_ioctl(struct file *file, unsigned int cmd,  unsigned long arg)
{
	...
	default:
		{
				DISPPR_ERROR("[%s]ioctl not supported, 0x%08x\n", __func__, cmd);
				return -ENOIOCTLCMD;
		}
	}
...
/*
* Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/02/01,
* add dre only use for camera
*/
extern void disp_aal_set_dre_en(int enable);
/*
* Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/01/27,
* add for ESS node
*/
unsigned int ess_mode = 1;
        ...
        struct device_attribute *attr, const char *buf, size_t num)
{
    ...
    */
    if (CABC_mode == 0) {
        ...
        * add for ESS node
        */
        if (is_project(OPPO_17197) && (ess_mode == 1)) {
            disp_aal_set_ess_en(0);
            printk("%s disable ESS\n", __func__);
        }
    } else {
    ...
    /*
    * Ling.Guo@PSW.MM.Display.LCD.Machine, 2018/01/29,
...
* add for ESS node
*/
static ssize_t LCM_ESS_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	printk("%s ess_mode=%d\n", __func__, ess_mode);
...
static ssize_t LCM_ESS_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t num)
{
	int ret;

	ret = kstrtouint(buf, 10, &ess_mode);

	printk("%s ess_mode=%d\n", __func__, ess_mode);

	if (ess_mode == 0) {
		disp_aal_set_ess_en(0);
		printk("%s disable ESS from update\n", __func__);
	} else if (ess_mode == 1) {
		disp_aal_set_ess_en(1);
		printk("%s enable ESS from update\n", __func__);
	}

	return num;
}

static DEVICE_ATTR(LCM_ESS, 0644, LCM_ESS_show, LCM_ESS_store);
...
/* LiPing-M@PSW.MultiMedia.Display.LCD.Machine.1077038, 2017/12/06, Add for Porting cabc interface */
	...
	*/
	if (is_project(OPPO_17197)) {
		...
		* add for ESS node
		*/
		ret = device_create_file(dev, &dev_attr_LCM_ESS);
		if (ret < 0)
		{
			printk("%s ESS device create file failed!\n", __func__);
		}
	}
