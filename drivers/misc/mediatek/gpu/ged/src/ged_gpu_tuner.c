...
static void _update_all_with_global_status(void)
{
	...
	list_for_each(listentry, &gItemList) {
		...
		if (item) {
			...
			GPU_TUNER_DEBUG("[%s] item feature(%08x) global feature(%8x)\n",
				__func__, item->status.feature, gpu_tuner_status.status.feature);
		}
	}
}
		...
		char *packagename)
{
		...
		list_for_each(listentry, &gItemList) {
			...
			item = list_entry(listentry, struct GED_GPU_TUNER_ITEM, List);
			if (item &&
				!(strncmp(item->status.packagename, packagename, strlen(item->status.packagename)))) {
				GPU_TUNER_DEBUG("[%s] package_name(%s) feature(%08x) value(%d)\n",
					__func__, item->status.packagename, item->status.feature, item->status.value);
				return item;
			}
		}
		...
		void *pvData)
{
		...
		list_for_each(listentry, &gItemList) {
			...
			if (item) {
				sprintf(buf, " [%d]\n", cnt++);
				seq_puts(psSeqFile, buf);
				sprintf(buf, " pkgname(%s) cmd(%s) feature(%08x)\n",
				item->status.packagename, (item->status.cmd) ? item->status.cmd : "N/A",
				item->status.feature);
				seq_puts(psSeqFile, buf);
				seq_puts(psSeqFile, "========================================\n");
			}
...
#define NUM_TOKEN 3
	...
	if (ret == NUM_TOKEN) {
		...
		strncpy(gpu_tuner_last_custom_hint.packagename, packagename, len);
		...
		GPU_TUNER_DEBUG("[last_hint] name(%s) cmd(%s) feature(%08x) value(%d)\n",
		gpu_tuner_last_custom_hint.packagename, gpu_tuner_last_custom_hint.cmd,
		gpu_tuner_last_custom_hint.feature, gpu_tuner_last_custom_hint.value);
		...
		void *pvData)
{
	...
	seq_puts(psSeqFile, "========================================\n");
	if (gpu_tuner_last_custom_hint.packagename) {
		...
		struct GED_GPU_TUNER_ITEM *status)
{
	...
	_tolower_s(p);
	if (!strncmp(packagename, global_packagename, strlen(global_packagename)))
		...
		enum GPU_TUNER_FEATURE eFeature)
{
	...
	if (!strncmp(packagename, global_packagename, strlen(global_packagename))) {
		...
		if (item) {
			...
			item =
			(struct GED_GPU_TUNER_ITEM *)ged_alloc_atomic(sizeof(struct GED_GPU_TUNER_ITEM));
	...
	if (item)
		GPU_TUNER_DEBUG("[%s]name(%s) feature(%d)\n",
			__func__, item->status.packagename, item->status.feature);
		...
		enum GPU_TUNER_FEATURE eFeature)
{
	...
	if (item)
		GPU_TUNER_DEBUG("[%s]name(%s) feature(%d) item(%p)\n",
			__func__, item->status.packagename, item->status.feature, item);
	GPU_TUNER_DEBUG("[%s]input feature(%08x) global status(%08x) err(%d)\n",
		__func__, eFeature, gpu_tuner_status.status.feature, err);
	}
...
GED_ERROR ged_gpu_tuner_init(void)
{
	...
	if (unlikely(err != GED_OK)) {
		GPU_TUNER_ERROR("[%s] failed to create custom hint set entry!\n",
		__func__);
...
int ged_bridge_gpu_tuner_status(GED_BRIDGE_IN_GPU_TUNER_STATUS *in, GED_BRIDGE_OUT_GPU_TUNER_STATUS *out)
{
	struct GED_GPU_TUNER_ITEM item;
