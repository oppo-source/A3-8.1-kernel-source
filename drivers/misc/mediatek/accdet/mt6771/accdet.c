...
#ifdef VENDOR_EDIT
/* xiang.fei@PSW.Multimedia.AudioDriver.HeadsetDet, 2018/06/15, Add for headset */
...
static int water_r = 10000; /* water resistor (10k ohm),custom define by DTS */
#endif
...
#ifdef VENDOR_EDIT
/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet.1222012, 2018/01/17,
...
static void accdet_pmic_Read_Efuse_HPOffset(void)
{
	...
	if (s_4_key_efuse_flag) {
		/* 4-key efuse:
		 * bit[9:2] efuse value is loaded, so every read out value need to be left shift 2 bit,
	...
	*/
	...
	ACCDET_INFO("[moisture_offset0]efuse=0x%x,moisture_offset=%d mv\n",
...
static int Accdet_PMIC_IMM_GetOneChannelValue(int key_check)
{
	...
	ACCDET_DEBUG("[accdet] adc_offset=%d mv,MIC_Vol=%d mv from API\n", g_accdet_auxadc_offset, vol_val);

	pmic_pwrap_write(AUXADC_RQST0, AUXADC_RQST_CH5_SET);
	mdelay(3);
	reg_val = pmic_pwrap_read(AUXADC_ADC5);
	while ((reg_val & AUXADC_DATA_RDY_CH5) != AUXADC_DATA_RDY_CH5)
		reg_val = pmic_pwrap_read(AUXADC_ADC5);

	vol_val = (pmic_pwrap_read(AUXADC_ADC5) & AUXADC_DATA_MASK);
	ACCDET_DEBUG("[accdet] vol_val=0x%x from AUXADC_ADC5\n", vol_val);
...
{
#ifdef VENDOR_EDIT
	/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet.1223267, 2018/01/14,
...
/* hw digital fast discharge bug, sw arround (2) : ACCDET_CON24[15:14]=00 and then wait xxms [15:14]=11b, [4]=1 ;
...
static inline void enable_accdet(u32 state_swctrl)
{
	...
	ACCDET_DEBUG("[enable_accdet]no HW MODE [0x%x]=0x%x\n", ACCDET_CON01, pmic_pwrap_read(ACCDET_CON01));
#endif
}
...
#ifndef HW_MODE_SUPPORT
	pmic_pwrap_write(ACCDET_CON01, pmic_pwrap_read(ACCDET_CON01) & (~(ACCDET_ENABLE_B0)));
	ACCDET_DEBUG("[disable_accdet]no HW MODE [0x%x]=0x%x\n", ACCDET_CON01, pmic_pwrap_read(ACCDET_CON01));
#endif
...
static void disable_micbias_callback(struct work_struct *work)
{
	...
	if (s_cable_type == HEADSET_NO_MIC) {
		...
		/* set PWM IDLE on */
#ifdef VENDOR_EDIT
		/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet, 2017/12/14, Modify
		 * for close idle */
		pmic_pwrap_write(ACCDET_CON02, reg_val & (~ACCDET_PWM_IDLE_B8_9_10));
#else /* VENDOR_EDIT */
		pmic_pwrap_write(ACCDET_CON02, reg_val|(~ACCDET_PWM_IDLE_B8_9_10));
...
/* Disable ACCDET */
		...
		/* set accdet pwm idle value = 0, otherwise if IdleValue=1, accdet pwm will lead to leakage
		* even we will disable Accdet pwm unit.
		*/
		reg_val = pmic_pwrap_read(ACCDET_CON02);
		pmic_pwrap_write(ACCDET_CON02,
				(reg_val&(~ACCDET_PWM_IDLE_B8_9_10)));
...
#ifdef VENDOR_EDIT
		/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet.1263175, 2018/02/05,
		 * add for hp delay detection */
		if (g_cur_eint_state == EINT_PIN_PLUG_IN) {
#ifdef VENDOR_EDIT
			/* Yongzhi.Zhang@PSW.MM.AudioDriver.Machine.1303468, 2018/03/27,
...
static int accdet_irq_handler(void)
{
	...
	unsigned int old_value1, old_value2, old_value3;
#endif
	...
	if (g_cur_eint_state == EINT_PIN_PLUG_OUT) {
		...
		pmic_pwrap_write(AUDENC_ANA_CON11, pmic_pwrap_read(AUDENC_ANA_CON11) & 0x1FFF);/*set bit[13-15] = 0*/
		...
		/* Enable moisture detection, set 239C bit[13] = 1*/
		pmic_pwrap_write(AUDENC_ANA_CON10, pmic_pwrap_read(AUDENC_ANA_CON10) | 0x2000);
		/* select PAD_HP_EINT for moisture detection, set 239C bit[14] = 0*/
		pmic_pwrap_write(AUDENC_ANA_CON10, pmic_pwrap_read(AUDENC_ANA_CON10) & 0xBFFF);
	...
	if ((reg_val & ACCDET_IRQ_B0)) {
		...
		*/
		...
		ACCDET_ERROR("[accdet_irq_handler] neither ACCDET IRQ nor EINT IRQ triggerred!!\n");
...
void accdet_init_once(int init_flag)
{
	...
	if (init_flag == ACCDET_INIT0_ONCE) {
		...
		pmic_pwrap_write(ACCDET_CON24, reg_val|ACCDET_HWEN_SEL_1
			|ACCDET_HWMODE_SEL|ACCDET_EINT0_DEB_OUT_DFF|ACCDET_EINIT_REVERSE);
		/* 20170904  bit7=1 to enable eint1 internal resistor*/
		pmic_pwrap_write(AUDENC_ANA_CON7, pmic_pwrap_read(AUDENC_ANA_CON7) | 0x80);
	#elif defined CONFIG_ACCDET_SUPPORT_BI_EINT
		/* L+G detect bit[1:0] = 2'b11 */
...
/* ======================config analog======================= */
...
#ifdef VENDOR_EDIT
		/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet, 2018/04/25,
		 * change VTH to 2V and internal eint resitance to 500k */
		/* select VTH for 2v, set 239E bit[10] = 1 */
		pmic_pwrap_write(AUDENC_ANA_CON11, pmic_pwrap_read(AUDENC_ANA_CON11) | 0x0400);
		/* xiang.fei@PSW.MM.AudioDriver.HeadsetDet, 2018/06/15, Modify for
			internal eint resitance 2M */
		...
		/* ACC mode: 0x239E [0:2] = 111b   [7]=0
		  ...
		  * 0x239E  bit11: eint config accdet,  bit7=1, must when bit11=1
		  * Set 0x239C bit8 = 1 to use internal bias for Mode6
		*/
		if (headset_dts_data.accdet_mic_mode == HEADSET_MODE_1) {
			reg_val &= 0xFF7F;
#ifndef VENDOR_EDIT
			/* Yongzhi.Zhang@PSW.MM.AudioDriver.HeadsetDet, 2018/02/12,
			 * modify for CT Test */
			...
			pmic_pwrap_write(AUDENC_ANA_CON11, reg_val|RG_ACCDET_MODE_ANA10_MODE6);
			reg_val = pmic_pwrap_read(AUDENC_ANA_CON10);
			pmic_pwrap_write(AUDENC_ANA_CON10, reg_val | 0x100);
		}
...
#ifdef HW_MODE_SUPPORT
#ifndef	DIGITAL_FASTDISCHARGE_SUPPORT
	pmic_pwrap_write(ACCDET_CON24, pmic_pwrap_read(ACCDET_CON24) & (~ACCDET_FAST_DISCAHRGE));
#else
	/* hw digital fast discharge bug, sw arround (1) : ACCDET_CON24[4] ACCDET_FAST_DISCAHRGE set to 0 initial;
	* please remember to find sw arround (2)
	*/
