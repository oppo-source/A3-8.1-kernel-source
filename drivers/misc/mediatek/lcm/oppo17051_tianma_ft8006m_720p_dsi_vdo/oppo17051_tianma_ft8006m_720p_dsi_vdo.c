...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	*/
	 ...
	 */
	register_device_proc("lcd", "ft8006m", "tianma vdo mode");
#endif
}
 ...
 */
int tianma_ft8006m_lcm_set_cabc_mode(void *handle,int mode)
{
	int mapped_level = 0;
...
extern unsigned int DSI_dcs_read_lcm_reg_v2_wrapper_DSI0(unsigned char cmd,
	unsigned char *buffer, unsigned char buffer_size);
...
int parse_reg_output(void *handle,const char *buf)
{
	...
	if (debug_read.length < 4) {
		retval = DSI_dcs_read_lcm_reg_v2_wrapper_DSI0(debug_read.buffer[0],
					debug_read.read_buffer, debug_read.length);
	} else {
		retval = DSI_dcs_read_lcm_reg_v3_wrapper_DSI0(debug_read.buffer[0],
					debug_read.read_buffer, debug_read.length);
	}
...
LCM_DRIVER oppo17051_tianma_ft8006m_720p_dsi_vdo_lcm_drv=
{
	.name = "oppo17051_tianma_ft8006m_720p_dsi_vdo",
