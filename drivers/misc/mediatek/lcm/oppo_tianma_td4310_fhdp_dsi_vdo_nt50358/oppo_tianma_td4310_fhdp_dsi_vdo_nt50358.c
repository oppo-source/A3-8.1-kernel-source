...
/*lcm id, for muti lcm use , need modify later*/
...
static void lcm_init(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		...
		LCM_LOGI("td4310----tps6132----lcm mode = cmd mode :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting, sizeof(init_setting) / sizeof(struct LCM_setting_table), 1);
		LCM_LOGI("td4310----tps6132----lcm mode = vdo mode :%d----\n", lcm_dsi_mode);
	}
}
...
LCM_DRIVER oppo_tianma_td4310_fhdp_dsi_vdo_nt50358_lcm_drv = {
	.name = "oppo_tianma_td4310_fhdp_dsi_vdo_nt50358_drv",
