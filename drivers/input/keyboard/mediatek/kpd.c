...
static void kpd_set_vol_key_state(int key, int key_val)
{
	...
	//pr_err("key:%d, key_val:%d, vol_key_password:0x%x\n",
		//key, key_val, vol_key_password);
}
...
/* Fuchun.Liao@BSP.CHG.Basic 2018/03/04 modify for enter dump */
	if (is_project(OPPO_17175) || is_project(OPPO_18011) || is_project(OPPO_18311)|| is_project(OPPO_18611)) {
		kpd_set_vol_key_state(kpd_dts_data.kpd_sw_rstkey, !pressed);
	}
	if ((is_project(OPPO_17175) || is_project(OPPO_18011) || is_project(OPPO_18311)|| is_project(OPPO_18611)) && aee_kpd_enable) {
...
static int init_custom_gpio_state(struct platform_device *client) {
	...
	/*for key volume up*/
    if (is_project(OPPO_17331) || is_project(OPPO_17197) || is_project(OPPO_17061) || (is_project(OPPO_18311) && (get_Operator_Version() != OPERATOR_18328_ASIA_SIMPLE_NORMALCHG))) {
		pr_err("%s not 17331 evb volume up key\n", __func__);
