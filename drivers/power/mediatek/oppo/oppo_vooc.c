...
static void oppo_vooc_fastchg_func(struct work_struct *work)
{
        ...
        int ret_info = 0;
        static bool isnot_power_on = true, fw_ver_info = false, adapter_fw_ver_info = false, data_err = false;
...
out:
        ...
        if ((data == VOOC_NOTIFY_VAL2 || (data_err && !phone_mcu_updated))
                && (chip->fastchg_dummy_started == false)) {
                oppo_chg_set_charger_type_unknown();
        ...
        if (data_err || (data == VOOC_NOTIFY_VAL2)) {
            chg_err("vooc_charge stop\n");
            switch_usb_state(0);
        }
#endif
}
