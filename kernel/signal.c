...
#ifdef VENDOR_EDIT
/* Fuchun.Liao@BSP.CHG.Basic 2018/01/03 for debug env set */
...
//Li.Liu@PSW.AD.Stability.Crash.1054829, 2016/10/08, Add for merging fangpan@oppo.com modify for the sender who kill system_server
        if(1) {
                ...
                if((sig == SIGHUP || sig == 33 || sig == SIGKILL || sig == SIGSTOP || sig == SIGABRT || sig == SIGTERM || sig == SIGCONT) && (!strcmp(t->comm, "system_server") || is_zygote_process(t))) {
                        //#ifdef VENDOR_EDIT
                        //Haoran.Zhang@PSW.AD.Stability.Crash.1054829, 2016/03/11, Modify for, to dump call stack of killing android core process.
                        dump_stack();
                        //#endif
                        printk("Some other process %d:%s want to send sig:%d %d:%s\n", current->pid, current->comm,sig, t->pid, t->comm);
                }
        }
#endif
